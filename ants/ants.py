...
class Insect:
    ...
    damage = 0
    is_waterproof=False# ADD CLASS ATTRIBUTES HERE
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def recursionfindingbees(thisshitplace, count):
            if thisshitplace.is_hive or (count>=self.lower_bound and count<=self.upper_bound):
                if thisshitplace.is_hive:
                    return None
                elif thisshitplace.bees==[]:
                    return recursionfindingbees(thisshitplace.entrance, count+1)
                else:
                    return random_bee(thisshitplace.bees)
            return recursionfindingbees(thisshitplace.entrance, count+1)
        return recursionfindingbees(self.place, 0)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        lst=list(range(len(self.place.bees)))
        L=list(map(lambda a: a-len(lst),lst))
        if amount>=self.health:
            for i in L:
                self.place.bees[i].reduce_health(amount+self.damage) 
        else:
            for i in L:
                self.place.bees[i].reduce_health(amount)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        def double_damage(p):
            if p.exit!=None:
                antbehind=p.exit.ant
                if isinstance(antbehind, ContainerAnt):
                    if hasattr(antbehind, 'damage'):
                        if not antbehind.damage_had_been_doubled:
                            antbehind.damage*=2
                            antbehind.damage_had_been_doubled=True
                    if antbehind.ant_contained!=None:
                        if hasattr(antbehind.ant_contained, 'damage'):
                            if not antbehind.ant_contained.damage_had_been_doubled:
                                antbehind.ant_contained.damage*=2
                                antbehind.ant_contained.damage_had_been_doubled=True
                                double_damage(p.exit)
                            else:
                                double_damage(p.exit)
                        else:
                            double_damage(p.exit)
