...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    paragraphs_that_qualify=[item for item in paragraphs if select(item)]
    return f"{paragraphs_that_qualify[k]}" if k < len(paragraphs_that_qualify) else ''
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def f(word):
        lst_of_word=split(remove_punctuation(lower(word)))
        return len([final_word for final_word in lst_of_word if final_word in subject])>0
    return f
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    i=0
    count=0
    for each_word in typed_words:
        if i>len(source_words)-1:
            break
        if each_word == source_words[i]:
            count+=1
        i+=1
